...
async function validateMunicipalityExists(
...
function addCompanyToStorage(company: any) {
  ...
  console.log(
    "🏢 Company added to storage. Total count:",
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        `✅ API: Successfully fetched ${companies.length} companies from database`
      );
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "🔍 POST /api/company - Cookie auth token:",
      token ? "Present" : "Missing"
    );
    console.log(
      "🔍 POST /api/company - All cookies:",
    ...
    if (!token) {
      console.log(
        "❌ POST /api/company - No authentication token found in cookies"
      );
      console.log(
        "❌ POST /api/company - Available cookies:",
        Array.from(cookieStore.getAll()).map((c) => c.name)
      );

      // In development mode, allow company creation without authentication for testing
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🔐 POST /api/company - Development mode: allowing unauthenticated company creation"
        );
        decoded = {
          id: "dev-user-id",
          role: "DEVELOPER",
        ...
        return NextResponse.json(
          {
            error:
              "Authentication required. No valid session found. Please log in again.",
          },
          { status: 401 }
        );
      }
    ...
    if (token && token.startsWith("mock-dev-token-")) {
      console.log(
        "🔐 POST /api/company - Mock token detected, allowing access"
      );
    ...
    const allowedRoles = ["SUPERADMIN", "MUNICIPAL_GOVERNMENTS", "DEVELOPER"];
    ...
    if (
      ...
      console.log(
        `❌ POST /api/company - Insufficient permissions. User role: ${userRole}, Allowed roles: ${allowedRoles.join(", ")}`
      );
      return NextResponse.json(
        {
    ...
    console.log(
      "🔍 POST /api/company - Skipping municipality validation for reliability"
    );
    console.log(
      "🔍 POST /api/company - Municipality ID provided:",
      body.municipalityId
    );

    // Create both User and Company records
    console.log(
      "🔍 POST /api/company - Creating company user and company data"
    );
    ...
    try {
      ...
      console.log(
        "✅ POST /api/company - Database connection test successful:",
        testConnection
      );
    } catch (dbError) {
      console.error(
        "❌ POST /api/company - Database connection test failed:",
        dbError
      );
      console.error(
        "❌ POST /api/company - Database URL exists:",
        !!process.env.DATABASE_URL
      );
      console.error(
        "❌ POST /api/company - Database URL prefix:",
        process.env.DATABASE_URL?.substring(0, 20) + "..."
      );
      throw new Error(
        "Database connection failed. Please check database setup."
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      console.log(
        "🔍 POST /api/company - Checking if municipality exists:",
        body.municipalityId
      );
      ...
      if (!dbMunicipality) {
        console.log(
          "⚠️ POST /api/company - Municipality not found in DB, creating it:",
          body.municipalityId
        );
        ...
        const municipalityData =
          defaultMunicipalities[
            body.municipalityId as keyof typeof defaultMunicipalities
          ];
        ...
        if (municipalityData) {
          ...
          console.log(
            "✅ POST /api/company - Municipality created in DB:",
            dbMunicipality.name
          );
        } else {
          console.error(
            "❌ POST /api/company - Unknown municipality ID:",
            body.municipalityId
          );
          throw new Error(
            `Municipality with ID '${body.municipalityId}' is not supported. Please select a valid municipality.`
          );
        }
      } else {
        console.log(
          "✅ POST /api/company - Municipality found in DB:",
          dbMunicipality.name
        );
      }
      ...
      if (existingUser) {
        console.error(
          "❌ POST /api/company - Username already exists:",
          body.username
        );
        throw new Error(
      ...
      if (existingCompany) {
        console.error(
          "❌ POST /api/company - Login email already exists:",
          body.email
        );
        throw new Error(
          `Email '${body.email}' is already registered for another company. Please use a different email.`
        );
      }

      // Create User record for authentication
      console.log(
        "🔐 POST /api/company - Creating User record for authentication"
      );
      ...
      console.log(
        "ℹ️ POST /api/company - Skipping Profile creation (not in current schema)"
      );

      // Create Company record with credentials
      console.log(
        "🔐 POST /api/company - Creating Company record with credentials"
      );
      ...
      console.log(
        "✅ POST /api/company - Company created with ID:",
        company.id
      );
    ...
    if (error && typeof error === "object" && "code" in error) {
      ...
      console.error(
        "❌ POST /api/company - Prisma error meta:",
        (error as any).meta
      );
      console.error(
        "❌ POST /api/company - Full Prisma error:",
    ...
    if (typeof body !== "undefined") {
      console.error(
        "❌ POST /api/company - Request body that caused error:",
        JSON.stringify(
          {
            ...body,
            password: "[REDACTED]",
    ...
    if (error instanceof Error) {
      if (error.message.includes("Municipality with ID")) {
        ...
        errorMessage =
          "A company with this information already exists. Please check your username and email.";
