...
export async function GET(request: NextRequest) {
  try {
    ...
    stats.forEach((stat) => {
      ...
      else if (stat.status === "UNDER_REVIEW")
        byStatus.underReview = stat._count.status;
      else if (stat.status === "PRE_SELECTED")
        byStatus.preSelected = stat._count.status;
      else if (stat.status === "REJECTED")
        byStatus.rejected = stat._count.status;
