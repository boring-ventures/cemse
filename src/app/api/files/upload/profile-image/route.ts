  ...
  try {
    ...
    if (token.includes(".") && token.split(".").length === 3) {
      ...
      if (tokenParts.length >= 4) {
        ...
        console.log(
          "🖼️ API: Database token validated for user:",
          decoded.username
        );
      }
    } else {
    ...
    console.log("🖼️ API: Form data received:", {
      hasFile: !!file,
      fileName: file?.name,
      fileSize: file?.size,
      fileType: file?.type,
    ...
    if (!file) {
      console.log("🖼️ API: No file found in form data");
      return NextResponse.json(
    ...
    console.log("🖼️ API: Updating profile with image URL:", imageUrl);
    ...
    try {
      ...
      console.log("🖼️ API: Profile updated successfully in database");
    } catch (dbError) {
      console.error("🖼️ API: Database update failed:", dbError);
      throw dbError;
    }
