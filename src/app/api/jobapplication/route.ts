...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        "✅ API: Successfully queried job applications, found:",
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "🔍 Checking if user profile exists for applicantId:",
      decoded.id
    );
    ...
    if (!userProfile) {
      console.error(
        "💼 API: Could not find or create user profile for applicantId:",
        decoded.id
      );
      return NextResponse.json(
        {
          message:
            "Could not create user profile. Please complete your profile first.",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      "💼 API: Job application created successfully:",
      jobApplication.id
    );
