...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "🔐 GET-TOKEN API - Available cookies:",
      allCookies.map((c) => c.name)
    );

    console.log(
      "🔐 GET-TOKEN API - cemse-auth-token value:",
    ...
    if (token.includes(".") && token.split(".").length === 3) {
      console.log(
        "🔐 GET-TOKEN API - JWT token found, returning for external API use"
      );
    ...
    console.log(
      "🔐 GET-TOKEN API - Database token found, but external API needs JWT"
    );
    return NextResponse.json(
