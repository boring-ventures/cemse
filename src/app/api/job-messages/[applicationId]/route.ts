...
function verifyToken(token: string) {
  try {
    console.log('🔍 Job Messages API - verifyToken - Attempting to verify token');
    
    // Handle mock development tokens
    if (token.startsWith('mock-dev-token-')) {
      console.log('🔍 Job Messages API - Mock development token detected');
    ...
    console.log('🔍 Job Messages API - Attempting JWT verification');
    const decoded = jwt.verify(token, JWT_SECRET) as any;
    console.log('🔍 Job Messages API - JWT verified successfully');
    return decoded;
  } catch (error) {
    console.log('🔍 Job Messages API - Token verification failed:', error instanceof Error ? error.message : 'Unknown error');
    return null;
  }
}
...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 GET Job Messages API - Application ID:', resolvedParams.applicationId);
    ...
    if (!token) {
      console.log('🔍 Job Messages API - No auth token found in cookies');
      return NextResponse.json({ error: 'No autorizado' }, { status: 401 });
    }

    const decoded = verifyToken(token);
    if (!decoded) {
      console.log('🔍 Job Messages API - Token verification failed');
      return NextResponse.json({ error: 'Token inválido' }, { status: 401 });
    }

    console.log('🔍 Job Messages API - User authenticated:', decoded.username || decoded.id);
    ...
    console.log('✅ Job Messages API - Successfully fetched', messages.length, 'messages');
    return NextResponse.json({ messages });
  } catch (error) {
    console.error('❌ Job Messages API - Error fetching messages:', error);
...
export async function POST(
  ...
  try {
    ...
    console.log('🔍 POST Job Messages API - Sending message for application:', resolvedParams.applicationId);
    console.log('🔍 POST Job Messages API - Message data:', body);
    ...
    if (!isApplicant && !isCompanyOwner && !isAdmin) {
      console.log('🔍 Job Messages API - Insufficient permissions for user:', decoded.username);
    ...
    console.log('🔍 Job Messages API - Creating message with:', {
      applicationId: resolvedParams.applicationId,
      senderId: decoded.id,
    ...
    console.log('✅ Job Messages API - Successfully created message:', newMessage.id);
    return NextResponse.json({ message: newMessage });
  } catch (error) {
    console.error('❌ Job Messages API - Error sending message:', error);
    return NextResponse.json(
