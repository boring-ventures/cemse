...
function verifyToken(token: string) {
  try {
    console.log(
      "🔍 Read Message API - verifyToken - Attempting to verify token"
    );
    ...
    console.log(
      "🔍 Read Message API - Token verification failed:",
...
export async function PUT(
  ...
  try {
    ...
    console.log(
      "🔍 PUT Read Message API - Marking message as read:",
    ...
    console.log(
      "🔍 Read Message API - User authenticated:",
    ...
    if (!isApplicant && !isCompanyOwner && !isAdmin) {
      console.log(
        "🔍 Read Message API - Insufficient permissions for user:",
        decoded.username
      );
      return NextResponse.json(
        { error: "Sin permisos para marcar este mensaje" },
        { status: 403 }
      );
    }
    ...
    console.log(
      "✅ Read Message API - Successfully marked message as read:",
      updatedMessage.id
    );
    return NextResponse.json({ message: updatedMessage });
  } catch (error) {
    console.error(
      "❌ Read Message API - Error marking message as read:",
      error
    );
    return NextResponse.json(
