...
function verifyToken(token: string) {
  try {
    ...
    console.log(
      "🔍 Mark Read API - Token verification failed:",
...
export async function PUT(
  ...
  try {
    ...
    console.log(
      "🔍 Mark Read API - User authenticated:",
    ...
    if (message.applicationId !== applicationId) {
      return NextResponse.json(
        { error: "Mensaje no pertenece a esta aplicación" },
        { status: 400 }
      );
    }
    ...
    if (!isApplicant && !isCompanyOwner && !isAdmin) {
      console.log(
        "🔍 Mark Read API - Insufficient permissions for user:",
        decoded.username
      );
      return NextResponse.json(
        { error: "Sin permisos para marcar el mensaje como leído" },
        { status: 403 }
      );
    }
    ...
    console.log(
      "✅ Mark Read API - Successfully marked message as read:",
      messageId
    );
    return NextResponse.json({
      success: true,
