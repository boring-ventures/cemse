...
export class ResourceService {
  ...
  static async getAllResources(filters?: any) {
    try {
      console.log(
        "🔍 ResourceService.getAllResources - Fetching resources with filters:",
        filters
      );
  ...
  static async uploadResource(formData: FormData) {
    try {
      console.log(
        "🔍 ResourceService.uploadResource - Uploading resource with file"
      );
  ...
  static async downloadResource(id: string) {
    const response = await apiCall(`/resource/${id}/download`, {
      method: "POST",
    });
    return response;
  }
