...
export const useLessonVideoChunkedUpload =
  (): UseLessonVideoChunkedUploadReturn => {
    ...
    const uploadVideoInChunks = async (
      ...
      if (file.size > 100 * 1024 * 1024) {
        console.log(
          "🎥 Hook: Very large video detected, starting with smaller chunks (128KB)"
        );
        chunkSize = 128 * 1024;
      }
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            if (retryCount <= maxRetries) {
              ...
              console.error(
                `🎥 Hook: Chunk ${chunkNumber} upload failed after ${maxRetries} retries:`,
                error
              );
            }
          }
        ...
        if (retryCount > maxRetries && lastError) {
          console.error(
            `🎥 Hook: Error uploading chunk ${chunkNumber}:`,
            lastError
          );
      ...
      while (finalizeRetryCount <= maxFinalizeRetries) {
        try {
          ...
          if (finalizeRetryCount <= maxFinalizeRetries) {
            ...
            // Wait before retry (exponential backoff)
            await new Promise((resolve) =>
              setTimeout(resolve, Math.pow(2, finalizeRetryCount) * 1000)
            );
          } else {
            console.error(
              `🎥 Hook: Finalize failed after ${maxFinalizeRetries} retries:`,
              error
            );
          }
        }
      ...
      throw new Error(
        `Failed to finalize upload after ${maxFinalizeRetries} retries: ${finalizeLastError?.message}`
      );
    };
    ...
    const uploadVideo = async (
      ...
      try {
        ...
        if (file.size > 100 * 1024 * 1024) {
          ...
          toast.info(
            `Archivo grande detectado (${fileSizeMB}MB). Usando carga por fragmentos para mejor confiabilidad.`
          );
        }
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Error al subir video de lección"
        );
        throw error;
      } finally {
